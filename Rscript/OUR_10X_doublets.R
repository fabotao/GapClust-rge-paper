...
for (q in 1:7){
  for (i in 1:20){
    ...
    k.num <- min(200, ifelse(ceiling(dim(tmp)[2]/10) < 12, 12, ceiling(dim(tmp)[2]/10))) 
    res <- GapClust::GapClust(data2, k=k.num)
    ...
    #plot(1:dim(diff.both)[2], skew, cex=0.3, col=ifelse(1:k.num %in% 1, 'red', 'black'))
    }
}
