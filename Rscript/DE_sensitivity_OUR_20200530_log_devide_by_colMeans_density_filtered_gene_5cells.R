...
load('../10X_DE_sensitivity/CD14_5_cells_1_ExprM.RData')
...
for(num in DE.range){
  ...
  for(k in 1:100){
    ...
    if(skew.max==1){
      top.cell <- which.min(v1.k[,(skew.max)])
    }else{
  ...
  write.csv(auc.ave, file='auc_5.csv')
}
...
save(auc.our, file='./results/AUC_res_OUR_20200530_log_devide_by_colMeans_density_filtered_gene_5cells.RData')
